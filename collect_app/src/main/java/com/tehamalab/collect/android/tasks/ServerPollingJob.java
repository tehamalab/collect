...
package com.tehamalab.collect.android.tasks;
...
import com.tehamalab.collect.android.activities.NotificationActivity;
...
import com.tehamalab.collect.android.utilities.NotificationUtils;

import com.tehamalab.collect.android.R;

import com.tehamalab.collect.android.dao.FormsDao;
import com.tehamalab.collect.android.logic.FormDetails;
...
public class ServerPollingJob extends Job {
    ...
    protected Result onRunJob(@NonNull Params params) {
        ...
        if (!formList.containsKey(DownloadFormListUtils.DL_ERROR_MSG)) {
            if (formList.containsKey(DownloadFormListUtils.DL_AUTH_REQUIRED)) {
                formList = downloadFormListUtils.downloadFormList(true);

                if (formList.containsKey(DownloadFormListUtils.DL_AUTH_REQUIRED) || formList.containsKey(DownloadFormListUtils.DL_ERROR_MSG)) {
                    return Result.FAILURE;
                }
            }
    ...
    private boolean wasThisNewerFormVersionAlreadyDetected(String formVersionHash) {
        Cursor cursor = new FormsDao().getFormsCursor(FormsProviderAPI.FormsColumns.LAST_DETECTED_FORM_VERSION_HASH + "=?", new String[]{formVersionHash});
    ...
    private void informAboutNewAvailableForms() {
        ...
        PendingIntent contentIntent = PendingIntent.getActivity(getContext(), ApplicationConstants.RequestCodes.FORM_UPDATES_AVAILABLE_NOTIFICATION, intent, PendingIntent.FLAG_UPDATE_CURRENT);

        NotificationUtils.showNotification(
                contentIntent,
                NotificationUtils.FORM_UPDATE_NOTIFICATION_ID,
    ...
    private void informAboutNewDownloadedForms(String title, HashMap<FormDetails, String> result) {
        ...
        PendingIntent contentIntent = PendingIntent.getActivity(getContext(), ApplicationConstants.RequestCodes.FORMS_DOWNLOADED_NOTIFICATION, intent, PendingIntent.FLAG_UPDATE_CURRENT);
    ...
    private void updateLastDetectedFormVersionHash(String formId, String formVersionHash) {
        ...
        values.put(FormsProviderAPI.FormsColumns.LAST_DETECTED_FORM_VERSION_HASH, formVersionHash);
        new FormsDao().updateForm(values, FormsProviderAPI.FormsColumns.JR_FORM_ID + "=?", new String[] {formId});
    }
